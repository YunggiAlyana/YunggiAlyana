# Hi, I'm Yunggi Alyana! 👋

📌 **Information Systems Student & Full-Stack Developer**

I'm an Information Systems student at **Universitas Terbuka** with a passion for building interactive web applications and exploring **Machine Learning**. Currently focused on integrating AI models into web environments (Next.js/Python) and crafting seamless user experiences.

---

## 🧰 Tech Stack

### **Frontend & Creative**
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=next.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat&logo=tailwind-css&logoColor=white)
![GSAP](https://img.shields.io/badge/GSAP-88CE02?style=flat&logo=greensock&logoColor=black)

### **Backend & Database**
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white)
![Hapi.js](https://img.shields.io/badge/Hapi.js-E95725?style=flat&logo=hapijs&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=flat&logo=firebase&logoColor=black)

### **Machine Learning & Python**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/Flask-000000?style=flat&logo=flask&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![Scikit-Learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)

---

## 🚀 Featured Projects

### 📡 TelcoSense (Capstone Project)
A product recommendation system for Telco services using **Machine Learning** to personalize user offers.
- **Tech:** React, Hapi.js, Python (FastAPI/ML).

### 📜 [CertifYT](https://github.com/YunggiAlyana/certifyt)
A web application to generate certificates based on YouTube playlist completion via **YouTube Data API v3**.
- **Tech:** Next.js, TypeScript.

### 📚 [Bookshelf API](https://github.com/YunggiAlyana/BOOKSHELF-API)
A backend service to manage book data with CRUD functionality.
- **Tech:** Node.js, Hapi.js.

### 🎨 [Contribution Painter](https://github.com/YunggiAlyana/Contribution-painter)
A unique web tool to design pixel art directly onto your GitHub contribution graph.
- **Tech:** HTML, JavaScript.

### 🎸 [Bocchi.rocks Clone](https://github.com/YunggiAlyana/bocchi-rocks-clone)
A recreation of interactive web animations inspired by the bocchi.rocks website.
- **Tech:** JavaScript, CSS Animations.

---

## 📬 Connect with Me

[![GitHub](https://img.shields.io/badge/GitHub-%23181717.svg?style=for-the-badge&logo=github&logoColor=white)](https://github.com/YunggiAlyana)
[![Email](https://img.shields.io/badge/Email-%23D14836.svg?style=for-the-badge&logo=gmail&logoColor=white)](mailto:yunggialyana123@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230A66C2.svg?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/yunggi-alyana318/)
